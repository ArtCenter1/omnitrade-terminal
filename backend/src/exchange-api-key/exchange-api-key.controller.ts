...
export class ExchangeApiKeyController {
  ...
  @Post()
  async addApiKey(@Request() req: ExpressRequest & { user: { user_id: string } }, @Body() dto: CreateExchangeApiKeyDto) {
    return this.apiKeyService.addApiKey(req.user.user_id, dto);
  }

  @Get()
  async listApiKeys(@Request() req: ExpressRequest & { user: { user_id: string } }) {
    return this.apiKeyService.listApiKeys(req.user.user_id);
  }

  @Delete(':id')
  async deleteApiKey(@Request() req: ExpressRequest & { user: { user_id: string } }, @Param('id') id: string) {
    return this.apiKeyService.deleteApiKey(req.user.user_id, id);
  }

  @Post(':id/test')
  async testApiKey(@Request() req: ExpressRequest & { user: { user_id: string } }, @Param('id') id: string, @Body() dto: TestExchangeApiKeyDto) {
