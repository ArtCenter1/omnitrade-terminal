...
const mockExchangeServiceImpl: MockExchangeService = {
  ...
  placeOrder(exchangeId: string, orderData: any): Order {
    logger.info('mockExchangeService placeOrder called', {
      component: 'mockExchangeService',
      method: 'placeOrder',
      data: {
        exchangeId,
        orderData,
    ...
    logger.debug('Using userId for order', {
      component: 'mockExchangeService',
      method: 'placeOrder',
      data: {
        userId,
        providedUserId: orderData.userId,
        usingDefault: !orderData.userId,
      },
    });

    // Ensure we have an orders array for this user
    if (!ordersCache[userId]) {
      logger.debug('Creating new orders array for user', {
        component: 'mockExchangeService',
        method: 'placeOrder',
        data: { userId },
      });
    ...
    logger.debug('Created new order object', {
      component: 'mockExchangeService',
      method: 'placeOrder',
      data: { order },
    });
    ...
    logger.debug('Added order to cache', {
      component: 'mockExchangeService',
      method: 'placeOrder',
      data: {
        userId,
        orderId: order.id,
        orderCount: ordersCache[userId].length,
      },
    });

    // For market orders, simulate immediate fill
    if (order.type === 'market') {
      logger.debug('Scheduling market order fill in 1 second', {
        component: 'mockExchangeService',
        method: 'placeOrder',
        data: {
          orderId: order.id,
        ...
        if (index !== -1) {
          logger.debug('Found order to fill', {
            ...
            data: {
              orderId: order.id,
              orderIndex: index,
              orderBeforeUpdate: ordersCache[userId][index],
            },
          });
          ...
          logger.info('Market order filled', {
            ...
            method: 'placeOrder',
            data: {
              filledOrder: ordersCache[userId][index],
              orderId: order.id,
              status: 'filled',
            },
          });
          ...
          logger.error('Could not find order to fill', {
            ...
            data: {
              orderId: order.id,
              availableOrderIds: ordersCache[userId].map((o) => o.id),
            },
          });
        }
      }, 1000);
