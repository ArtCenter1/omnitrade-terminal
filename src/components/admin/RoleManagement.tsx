...
const RoleManagement: React.FC<RoleManagementProps> = ({ hideHeader = false }) => {
  ...
  return (
    <div className="p-4">
      ...
      <h3 className="font-semibold mb-3 text-lg border-b border-gray-700 pb-2">Roles</h3>
      <ul className="mb-6 grid grid-cols-1 gap-4">
        {roles.map((role) => (
          <li key={role.role_id} className="bg-gray-800 p-4 rounded-lg border border-gray-700 hover:border-gray-600 transition-colors">
            <div className="flex justify-between items-center">
              ...
              <div>
                ...
                >
                  {selectedRoleId === role.role_id ? 'Close' : 'Manage Permissions'}
                </button>
                <button
              ...
              {role.permissions.length > 0 ? (
                <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2">
                  {role.permissions.map((perm) => (
                    <div key={perm.permission_id} className="flex items-center justify-between bg-gray-700 text-gray-200 p-2 rounded">
                      <span className="font-mono text-sm">{perm.resource}:{perm.action}</span>
                      <button
                        onClick={() => handleRemovePermission(role.role_id, perm.permission_id)}
                ...
                <p className="text-gray-500 italic mt-2">No permissions assigned to this role.</p>
              )}
            </div>

            {selectedRoleId === role.role_id && (
              <div className="mt-4 p-3 bg-gray-100 rounded text-gray-800">
                <h4 className="font-medium mb-2">Assign Permissions to {role.name}</h4>
                <div className="max-h-40 overflow-y-auto">
                  {permissions
                    .filter(p => !role.permissions.some(rp => rp.permission_id === p.permission_id))
                    .map(perm => (
                      <div key={perm.permission_id} className="flex justify-between items-center mb-1 p-1 hover:bg-gray-200 border border-gray-200 rounded">
                        <div>
                          <strong className="text-gray-800">{perm.resource}:{perm.action}</strong>
                          {perm.description && <p className="text-xs text-gray-600">{perm.description}</p>}
                        </div>
                        <button
                          onClick={() => handleAssignPermission(role.role_id, perm.permission_id)}
                  ...
                  {permissions.filter(p => !role.permissions.some(rp => rp.permission_id === p.permission_id)).length === 0 && (
                    <p className="text-gray-500 italic">All available permissions are already assigned to this role.</p>
                  )}
                </div>
              </div>
            )}
      ...
      <h3 className="font-semibold mb-3 text-lg border-b border-gray-700 pb-2">Available Permissions</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {permissions.map((perm) => (
          <div key={perm.permission_id} className="bg-gray-800 p-3 rounded-lg border border-gray-700 flex flex-col">
            ...
            {perm.description && <p className="text-sm text-gray-400">{perm.description}</p>}
          </div>
        ))}
      </div>
    </div>
