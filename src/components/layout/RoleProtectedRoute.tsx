...
(window as any).toggleRoleDebug = () => {
  ...
  console.log(
    `Role debug panel is now ${newValue === "true" ? "hidden" : "visible"}`
  );
...
export const RoleProtectedRoute: React.FC<RoleProtectedRouteProps> = ({
  ...
  const RoleDebugPanel = ({
    ...
    setDebugHidden: React.Dispatch<React.SetStateAction<boolean>>;
  }) => {
    ...
    return (
      ...
      >
        ...
        <p>
          Role: <span className="font-semibold text-green-400">{userRole}</span>
        </p>
  ...
  const DebugInfo = () => {
    ...
    return debugHidden ? (
      <RoleDebugToggle setDebugHidden={setDebugHidden} />
    ) : (
      <RoleDebugPanel setDebugHidden={setDebugHidden} />
    );
  };
