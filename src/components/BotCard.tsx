    ...
    <div className="bg-gray-900 rounded-lg overflow-hidden">
      <div className="p-6">
        <div className="flex items-start mb-3">
          <div className={`w-12 h-12 ${bot.iconBg} rounded-full flex items-center justify-center mr-4`}>
            <img src={bot.icon || "/placeholder.svg"} alt={bot.title} className="w-6 h-6" />
          </div>
          <div>
            <div className="flex items-center">
              <h3 className="text-white font-medium">{bot.title}</h3>
              {bot.tags?.map((tag, index) => (
                <span key={index} className="ml-2 text-xs bg-yellow-500 text-black px-2 py-0.5 rounded">
                  {tag}
                </span>
              ))}
            </div>
