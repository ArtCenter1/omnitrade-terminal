...
const GraphPlaceholder = () => (
  <svg width="100%" height="100%" viewBox="0 0 400 200" preserveAspectRatio="xMidYMid meet" className="text-gray-600">
    {/* Grid lines */}
    <line x1="50" y1="10" x2="50" y2="170" stroke="currentColor" strokeWidth="0.5" strokeDasharray="2,2" />
    <line x1="150" y1="10" x2="150" y2="170" stroke="currentColor" strokeWidth="0.5" strokeDasharray="2,2" />
    <line x1="250" y1="10" x2="250" y2="170" stroke="currentColor" strokeWidth="0.5" strokeDasharray="2,2" />
    <line x1="350" y1="10" x2="350" y2="170" stroke="currentColor" strokeWidth="0.5" strokeDasharray="2,2" />
    <line x1="30" y1="40" x2="370" y2="40" stroke="currentColor" strokeWidth="0.5" strokeDasharray="2,2" />
    <line x1="30" y1="90" x2="370" y2="90" stroke="currentColor" strokeWidth="0.5" strokeDasharray="2,2" />
    <line x1="30" y1="140" x2="370" y2="140" stroke="currentColor" strokeWidth="0.5" strokeDasharray="2,2" />

    {/* Axes */}
    <line x1="30" y1="170" x2="370" y2="170" stroke="currentColor" strokeWidth="1" /> {/* X-axis */}
    <line x1="30" y1="10" x2="30" y2="170" stroke="currentColor" strokeWidth="1" /> {/* Y-axis */}

    {/* Placeholder Data Lines */}
    <polyline points="50,150 100,100 150,120 200,80 250,90 300,50 350,60" fill="none" stroke="#8884d8" strokeWidth="2" />
    <polyline points="50,160 100,140 150,150 200,110 250,130 300,100 350,120" fill="none" stroke="#82ca9d" strokeWidth="2" />

    {/* Axis Labels (simplified) */}
    <text x="190" y="190" fontSize="10" fill="currentColor" textAnchor="middle">Time</text>
    <text x="15" y="95" fontSize="10" fill="currentColor" textAnchor="middle" transform="rotate(-90 15,95)">PNL</text>
  </svg>
);
...
export function CommunityArena() {
  ...
  const handleRunSimulation = () => {
    if (selectedBot1 && selectedBot2) {
      ...
      console.log(`Simulating battle between ${selectedBot1} and ${selectedBot2}`);
      // Simulate loading
      setTimeout(() => {
  ...
  const bot1Name = placeholderBots.find(b => b.id === selectedBot1)?.name || 'Bot 1';
  const bot2Name = placeholderBots.find(b => b.id === selectedBot2)?.name || 'Bot 2';
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="md:col-span-1 space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            {/* Bot Selector 1 */}
            <div>
              <label htmlFor="bot1-select" className="block text-sm font-medium text-gray-300 mb-1">Select Challenger 1</label>
              <Select onValueChange={setSelectedBot1} value={selectedBot1 ?? undefined} disabled={isRunning}>
                <SelectTrigger id="bot1-select">
                  <SelectValue placeholder="Choose a bot..." />
                </SelectTrigger>
                <SelectContent>
                  {placeholderBots.map((bot) => (
                    <SelectItem key={bot.id} value={bot.id} disabled={bot.id === selectedBot2}>
                      {bot.name}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            {/* Bot Selector 2 */}
            <div>
              <label htmlFor="bot2-select" className="block text-sm font-medium text-gray-300 mb-1">Select Challenger 2</label>
              <Select onValueChange={setSelectedBot2} value={selectedBot2 ?? undefined} disabled={isRunning}>
                <SelectTrigger id="bot2-select">
                  <SelectValue placeholder="Choose another bot..." />
                </SelectTrigger>
                <SelectContent>
                  {placeholderBots.map((bot) => (
                    <SelectItem key={bot.id} value={bot.id} disabled={bot.id === selectedBot1}>
                      {bot.name}
                    </SelectItem>
                  ))}
                </SelectContent>
      ...
      <div className="md:col-span-2">
        <Card className="h-full flex flex-col"> {/* Make card fill height and use flex column */}
          <CardHeader>
          ...
          <CardContent className="flex-grow flex items-center justify-center"> {/* Center content vertically and horizontally */}
            {!showResults && (
