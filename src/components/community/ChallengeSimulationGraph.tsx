...
export function ChallengeSimulationGraph() {
  ...
  const [challengeResult, setChallengeResult] = React.useState<ChallengeResult | null>(null);
  ...
  return (
    <Card>
      ...
      <CardContent className="h-64 flex flex-col justify-between items-center overflow-hidden"> {/* Adjust height & layout, center items */}
        {challengeResult ? (
          <div className="flex-grow w-full flex items-center justify-center">
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <YAxis label={{ value: 'Cumulative PnL (USDT)', angle: -90, position: 'insideLeft' }}/> {/* Y-axis is PnL */}
                <Tooltip formatter={(value: number) => `${value.toFixed(2)} USDT`} /> {/* Format tooltip */}
                <Legend />
                {/* Use userPnl and opponentPnl data keys */}
                <Line type="monotone" dataKey="userPnl" stroke="#8884d8" activeDot={{ r: 8 }} name="Your Bot PnL" />
                <Line type="monotone" dataKey="opponentPnl" stroke="#82ca9d" name={`${challengeResult?.opponentName || 'Opponent'} PnL`} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        ) : (
         ...
         <Button variant="outline" size="sm" className="mt-2" onClick={() => simulateChallenge('CryptoKing')}>
