  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Performance distribution across all participants.</CardDescription>
        {/* Optional: Add specific rank like "#3665 of 4507" if data is available */}
      </CardHeader>
      <CardContent className="h-64"> {/* Adjust height as needed */}
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            <CartesianGrid strokeDasharray="3 3" stroke="#444" vertical={false} /> {/* Faint horizontal grid */}
            <XAxis
              dataKey="range"
            ...
            <Tooltip
              ...
              contentStyle={{ backgroundColor: '#1f2937', border: 'none', borderRadius: '4px' }} // Tooltip style
              labelStyle={{ color: '#e5e7eb' }}
              itemStyle={{ color: '#9ca3af' }}
            />
            <Bar dataKey="count" name="Bots in Range" fill="#dc2626"> {/* Default bar color (red) */}
              {performanceData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={entry.range === userBotPnlRange ? '#22c55e' : '#dc2626'} /> // Highlight user's range (green)
              ))}
            </Bar>
