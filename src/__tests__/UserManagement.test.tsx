...
describe('UserManagement', () => {
  ...
  it('assigns a role to a user', async () => {
    ...
    fireEvent.change(screen.getByTestId('assign-role-select'), { target: { value: 'r2' } });

    await waitFor(() => expect(adminApi.assignRoleToUser).toHaveBeenCalledWith('u1', 'r2'));
    await waitFor(() => expect(screen.getByText(/Role assigned successfully/i)).toBeInTheDocument());
  });
  ...
  it('removes a role from a user', async () => {
    ...
    await waitFor(() => expect(adminApi.removeRoleFromUser).toHaveBeenCalledWith('u1', 'r1'));
    await waitFor(() => expect(screen.getByText(/Role removed successfully/i)).toBeInTheDocument());
  });

  it('shows error message on API failure', async () => {
    (adminApi.fetchUsers as any).mockRejectedValueOnce(new Error('Fetch failed'));
    (adminApi.fetchRoles as any).mockResolvedValue(mockRoles);
