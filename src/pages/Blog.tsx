...
const blogPosts = [
  {
    ...
    excerpt: "Learn how Cody AI analyzes historical data to predict potential market shifts...",
  ...
  },
  {
  ...
  },
   {
    ...
    excerpt: "Protect your capital with stop-loss orders, position sizing, and portfolio diversification...",
    imageUrl: "/placeholder.svg", // Replace with actual image path
      ...
      <main className="flex-grow container mx-auto px-4 py-12 md:py-16">
        ...
        <section className="text-center mb-12 md:mb-16">
          ...
          <div className="mt-8 max-w-md mx-auto relative">
            <Input type="search" placeholder="Search articles..." className="pl-10 bg-gray-800 border-gray-700 focus:border-purple-500 focus:ring-purple-500" />
        ...
        <section>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {blogPosts.map((post) => (
              <div key={post.id} className="bg-gray-900/50 rounded-lg overflow-hidden shadow-lg border border-gray-800 flex flex-col">
                <img src={post.imageUrl} alt={post.title} className="w-full h-48 object-cover" />
                <div className="p-6 flex flex-col flex-grow">
                  <div className="mb-4">
                    <span className="inline-block bg-purple-600/20 text-purple-400 text-xs font-semibold px-2 py-1 rounded mr-2">{post.category}</span>
                    <span className="text-gray-500 text-xs">{post.date} · {post.readTime}</span>
                  </div>
                  <h3 className="text-xl font-semibold mb-2 hover:text-purple-400 transition-colors cursor-pointer">{post.title}</h3>
                  <p className="text-gray-400 text-sm mb-4 flex-grow">{post.excerpt}</p>
                  <Button variant="link" className="p-0 text-purple-400 hover:text-purple-300 self-start group">
