  ...
  try {
    ...
    if (savedData) {
      try {
        ...
        if (parsedData && parsedData.users && parsedData.roles && parsedData.permissions) {
          return parsedData;
        } else {
          console.warn('Invalid mock data structure in localStorage, using default data');
        }
      } catch (parseError) {
    ...
    users: [
      {
        ...
        roles: [
          { role_id: 'r1', name: 'Admin', description: 'Administrator role', permissions: [] }
        ]
      },
      {
        user_id: 'u2',
        ...
        roles: [
          { role_id: 'r2', name: 'User', description: 'Regular user role', permissions: [] }
        ]
      }
    ],
    roles: [
      {
        ...
        permissions: [
          { permission_id: 'p1', action: 'read', resource: 'users', description: 'Read users' },
          { permission_id: 'p2', action: 'write', resource: 'users', description: 'Write users' },
          { permission_id: 'p3', action: 'read', resource: 'roles', description: 'Read roles' },
          { permission_id: 'p4', action: 'write', resource: 'roles', description: 'Write roles' }
        ]
      },
      {
        role_id: 'r2',
      ...
      },
      {
        ...
        permissions: [
          { permission_id: 'p1', action: 'read', resource: 'users', description: 'Read users' },
          { permission_id: 'p5', action: 'read', resource: 'premium', description: 'Access premium features' }
        ]
      }
    ],
    permissions: [
...
const saveMockData = () => {
  try {
    ...
    const currentUserEmail = localStorage.getItem('userEmail') || 'artcenter1@gmail.com';
    ...
    if (currentUser) {
      ...
      const hasPremiumRole = currentUser.roles.some(r => r.name === 'Premium');
      if (hasPremiumRole) {
...
export function setupMockAdminApi() {
  ...
  window.fetch = async function(input: RequestInfo, init?: RequestInit) {
    ...
    if (url.match(/\/api\/permissions\/.*/) && init?.method === 'DELETE') {
      ...
      const index = mockPermissions.findIndex(p => p.permission_id === permissionId);
