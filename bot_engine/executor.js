...
try {
  ...
  async job => {
    ...
    console.log(`[Executor] Starting bot ${botId} for user ${userId} (trigger: ${trigger})`);
    ...
    try {
      ...
      if (bot.consecutiveFailures >= FAILURE_THRESHOLD) {
        ...
        console.warn(`[Executor] Bot ${botId} disabled after ${FAILURE_THRESHOLD} failures`);
      }

      throw error; // Let BullMQ handle retries
    }
  },
  { connection, concurrency: 5, attempts: 3, backoff: { type: 'exponential', delay: 1000 } }
  );
